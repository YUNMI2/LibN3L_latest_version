...
public:
  ...
  inline void initial(const NRMat<dtype>& wordEmb) {
    ...
    _meanE = NewTensor<xpu>(Shape2(_nVSize, _nDim), d_zero);
...
/*  void updateAdaGrad(dtype regularizationWeight, dtype adaAlpha, dtype adaEps) {
  ...
  inline void updateAdaGrad(dtype regularizationWeight, dtype adaAlpha, dtype adaEps){
    ...
    for (it = _indexers.begin(); it != _indexers.end(); ++it) {
      int index = *it;
      _gradE[index] = _gradE[index] + _E[index] * regularizationWeight;
      _eg2E[index] = _eg2E[index] + _gradE[index] * _gradE[index];
      _E[index] = _E[index] - _gradE[index] * adaAlpha / F<nl_sqrt>(_eg2E[index] + adaEps); 
    }      
  ...
  inline void updateAdam(dtype belta1, dtype belta2, dtype regularizationWeight, dtype adaAlpha, dtype adaEps){
    ...
    for (it = _indexers.begin(); it != _indexers.end(); ++it) {
      int index = *it;
      _gradE[index] = _gradE[index] + _E[index] * regularizationWeight;
      _meanE[index] = belta1 * _meanE[index] + (1 - belta1) * _gradE[index];
      _eg2E[index] = belta2 * _eg2E[index] + (1 - belta2) * _gradE[index] * _gradE[index];
      lr_t = adaAlpha * sqrt (1 - pow(belta2, _last_update[index] + 1)) / (1 - pow(belta1, _last_update[index] + 1 ));
     _E[index] = _E[index] - _meanE[index] * lr_t / F<nl_sqrt>(_eg2E[index] + adaEps); 
     _last_update[index]++;
    }
    clearGrad();
  }  
