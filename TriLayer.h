...
public:
  ...
  Tensor<xpu, 2, dtype> _meanW1;
  Tensor<xpu, 2, dtype> _meanW2;
  Tensor<xpu, 2, dtype> _meanW3;
...
public:
  ...
  inline void initial(int nOSize, int nISize1, int nISize2, int nISize3, bool bUseB = true, int seed = 0, int funcType = 0) {
    ...
    _meanW1 = NewTensor<xpu>(Shape2(nOSize, nISize1), d_zero);
    ...
    _meanW2 = NewTensor<xpu>(Shape2(nOSize, nISize2), d_zero);
    ...
    _meanW3 = NewTensor<xpu>(Shape2(nOSize, nISize3), d_zero);
...
public:
  ...
  inline void updateAdam(dtype belta1, dtype belta2, dtype regularizationWeight, dtype adaAlpha, dtype adaEps){   
    ...
    _gradW1 = _gradW1 + _W1 * regularizationWeight;    
    _meanW1 = belta1 * _meanW1 + (1 - belta1) * _gradW1;
    _eg2W1 = belta2 * _eg2W1 + (1 - belta2)* _gradW1 * _gradW1;
    _W1 = _W1 - _meanW1 * lr_t/F<nl_sqrt>(_eg2W1 + adaEps);
		          
    _gradW2 = _gradW2 + _W2 * regularizationWeight;    
    _meanW2 = belta1 * _meanW2 + (1 - belta1) * _gradW2;
    _eg2W2 = belta2 * _eg2W2 + (1 - belta2)* _gradW2 * _gradW2;
    _W2 = _W2 - _meanW2 * lr_t/F<nl_sqrt>(_eg2W2 + adaEps);
    
    _gradW3 = _gradW3 + _W3 * regularizationWeight;    
    _meanW3 = belta1 * _meanW3 + (1 - belta1) * _gradW3;
    _eg2W3 = belta2 * _eg2W3 + (1 - belta2)* _gradW3 * _gradW3;
    _W3 = _W3 - _meanW3 * lr_t/F<nl_sqrt>(_eg2W3 + adaEps);
  ...
  void writeModel(LStream &outf) {
    ...
    SaveBinary(outf, _meanW1);
    SaveBinary(outf, _meanW2);
    SaveBinary(outf, _meanW3);
  ...
  void loadModel(LStream &inf) {
    ...
    LoadBinary(inf, &_meanW1, false);
    LoadBinary(inf, &_meanW2, false);
    LoadBinary(inf, &_meanW3, false);
