...
public:
  ...
  inline void initial(int nOSize, int nISize, bool bUseB = true, int seed = 0, int funcType = 0) {
    ...
    _meanW = NewTensor<xpu>(Shape2(nOSize, nISize), d_zero);
...
public:
  ...
  inline void updateAdam(dtype belta1, dtype belta2, dtype regularizationWeight, dtype adaAlpha, dtype adaEps){
    ...
    _gradW = _gradW + _W * regularizationWeight;
    _meanW = belta1 * _meanW + (1 - belta1) * _gradW;
    _eg2W = belta2 * _eg2W + (1 - belta2)* _gradW * _gradW;
    _W = _W - _meanW * lr_t/F<nl_sqrt>(_eg2W + adaEps);
