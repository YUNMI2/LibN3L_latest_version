...
inline void windowlized(const vector<Tensor<xpu, 2, dtype> > &wi, vector<Tensor<xpu, 2, dtype> > &wo, int context)
{
  ...
  int dim1 = wi[0].size(0), dim2 = wi[0].size(1);//(1,150)
  int odim1 = wo[0].size(0), odim2 = wo[0].size(1);//(1,750)
  int computeddim2 = (2 * context + 1) * dim2;//750
  ...
  for (int idx = 0; idx < seqsize; idx++) {
    wo[idx] = 0.0;
    offset = 0;
    for (int idp = idx - context; idp <= idx + context; idp++) {//-2 ~ +2，move 4
      if (idp < 0 || idp >= seqsize) {
        offset += dim2;//offset 从
      } else {
