...
public:
  ...
  Tensor<xpu, 2, dtype> _mean_tagBigram;
...
public:
  ...
  inline void initial(int nLabelSize, int seed = 0) {
    ...
    _mean_tagBigram = NewTensor<xpu>(Shape2(nLabelSize, nLabelSize), d_zero);
...
public:
  ...
  inline void updateAdam(dtype belta1, dtype belta2, dtype regularizationWeight, dtype adaAlpha, dtype adaEps){
    ...
    _grad_tagBigram = _grad_tagBigram + _tagBigram * regularizationWeight;
    _mean_tagBigram = belta1 * _mean_tagBigram + (1 - belta1) * _grad_tagBigram;
    _eg2_tagBigram = belta2 * _eg2_tagBigram + (1 - belta2)* _grad_tagBigram * _grad_tagBigram;
    _tagBigram = _tagBigram - _mean_tagBigram * lr_t/F<nl_sqrt>(_eg2_tagBigram + adaEps);
  ...
  void writeModel(LStream &outf) {
    ...
    SaveBinary(outf, _mean_tagBigram);
  }
  ...
  void loadModel(LStream &inf) {
    ...
    LoadBinary(inf, &_mean_tagBigram, false);
  }
